...
{
    /// <summary>
    /// [EXPERIMENTAL] Counts blocks mined/broken matching a code pattern.
    /// Args: [0] questId, [1] objectiveId, [2] blockCode (supports * wildcard suffix), [3] need
    /// </summary>
    public class MineBlockObjective : ActionObjectiveBase
    {
        private const string EventName = "mineblock";
        ...
        /// <summary>
        /// Called when a block is broken. Checks if it matches the target code.
        /// </summary>
        public void TryIncrement(IPlayer byPlayer, string brokenBlockCode, string questId, string objectiveId, string targetBlockCode, int need)
        {
            if (byPlayer?.Entity?.WatchedAttributes == null) return;
            if (string.IsNullOrWhiteSpace(brokenBlockCode)) return;

            if (!BlockCodeMatches(brokenBlockCode, targetBlockCode)) return;
        ...
        private static bool BlockCodeMatches(string brokenCode, string targetCode)
        {
            ...
            if (targetCode.EndsWith("*"))
            {
                ...
                return brokenCode.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)
                    || brokenCode.Contains(prefix, StringComparison.OrdinalIgnoreCase);
            }

            return string.Equals(brokenCode, targetCode, StringComparison.OrdinalIgnoreCase)
                || brokenCode.Contains(targetCode, StringComparison.OrdinalIgnoreCase);
        }
